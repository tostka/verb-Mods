...
#*------v test-ModuleDevFunctionSync.ps1 v------
...
#*------v uninstall-ModuleAllVersionsTDO.ps1 v------
function uninstall-ModuleAllVersionsTDO {
    ...
    * 8:49 AM 6/30/2026 init
    ...
    .EXAMPLE
    PS> $psdeskmods ="Pester","PSScriptAnalyzer" ; 
    PS> $bRet = uninstall-ModuleAllVersionsTDO -Modules $psdeskmods -scope CurrentUser -verbose -whatif:$($whatif) ; 
    Install Pester & PSScriptAnalyzer mods to the CurrrentUser context, with verbose output, and whatif read from a preset
    .EXAMPLE
    PS> $bRet = uninstall-ModuleAllVersionsTDO -Modules 'Statistics' -RequiredVersion '1.2.0.149' -scope CurrentUser -verbose -whatif:$($whatif) ; 
    Demo installing the Statistics module with a specified RequiredVersion, to CurrentUser scope, verbose output, and whatif
    .EXAMPLE
    PS> uninstall-ModuleAllVersionsTDO -Modules 'AzureAD;2.0.2.182','ExchangeOnlineManagement;3.6.0' -Scope AllUsers -Repository PSGallery -verbose -whatif ;
    Demo installing array of AzureAD & EOM modules, each with a RequiredVersion specified (after semicolon delimiter), to AllUsers scope, from PSGallery repo, with verbose outputs, and whatif specified
    .EXAMPLE
    PS> $whatif = $true ; 
    PS> $trepo = 'RepoX' ; 
    ...
    PS>     $tmods += New-Object -TypeName PsObject -Property $hshMod ; 
    PS> } ; 
    PS> write-verbose "reduce to only the latest vers of each module, build a 'modname;modversion' string to feed into uninstall-ModuleAllVersionsTDO()" ; 
    PS> $procmods = $tmods.module | select -unique | %{
    PS>     $thisUmod = $_ ; 
    PS>     $tmods | ?{$_.module -eq $thisUmod} | sort version | select -last 1 | %{ "$($_.module);$($_.version)" | write-output } ; 
    PS> } ; 
    ...
    PS>     whatif = $($whatif) ;
    PS> }
    PS> write-host -foregroundcolor green "$((get-date).ToString('HH:mm:ss')):uninstall-ModuleAllVersionsTDO w`n$(($pltIsMod|out-string).trim())" ; 
    PS> uninstall-ModuleAllVersionsTDO @pltIsMod ; 
    .LINK
    https://github.com/tostka/verb-mods
    #>
    [CmdletBinding()] 
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUv0OEnD/JnPvcgx6bpIkvVQsj
# Zi+gggI4MIICNDCCAaGgAwIBAgIQWsnStFUuSIVNR8uhNSlE6TAJBgUrDgMCHQUA
...
# CisGAQQBgjcCAQsxDjAMBgorBgEEAYI3AgEVMCMGCSqGSIb3DQEJBDEWBBSQMRLw
# HONF8GFGhgDSfLRWlQGilzANBgkqhkiG9w0BAQEFAASBgCNEL8bOqJR5qZ2UzBZY
# fJN9C9bAOtXqALjKeaEitThmzOejVGnPW1cC5/rkeMucoJUI98E2WpFGU5fsbqB7
# aA3Q+LoB+axplNBQJrE/4K2uYCWSXzxYufZucuh3TLG6f+3yI0398r8cw0p77Ylq
# B1wcBilEuCPAG0WdG1TWOd/3
